class Sboxtrails():
    ...
    def CreateDivisionTrails(self):
        ...
        for item in INDP:
            ...
            t1 = 0
            t2 = 0
            for k in range(int(len(item) / 2)):
                t1 = t1 + item[k] * (2 ** (int(len(item) / 2) - k - 1))
            INDP1.append(t1)
            for k in range(int(len(item) / 2), len(item)):
                t2 = t2 + item[k] * (2 ** (len(item) - k - 1))
            INDP1.append(t2)
            # print(INDP1)
            INDP2.append(INDP1)
    ...
    def PrintfDivisionTrails(self):
        """
		Write all division trails of an sbox into a file
		"""
        INDP = self.CreateDivisionTrails()
        # print("Division Trails of sbox:")
        for l in INDP:
            # print(str(l))
            self.sboxtrail.append(l)
        return self.sboxtrail

        # print(self.sboxtrail)
        # for item in self.sboxtrail:
        #     print(item)
        #     print(item[0:4])
        # print(len(item))
